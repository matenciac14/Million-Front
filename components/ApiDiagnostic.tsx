...
export const ApiDiagnostic: React.FC = () => {
  ...
  const testEndpoint = async (url: string, description: string) => {
    addResult({ endpoint: description, status: 'pending', message: 'Probando...' });
    ...
    try {
      ...
      addResult({
        ...
        message: `${response.status} ${response.statusText} (${endTime - startTime}ms)`,
        data: data
      });
    } catch (error) {
      addResult({
  ...
  const runDiagnostic = async () => {
    ...
    await testEndpoint(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.PROPERTIES}`, 'Properties (sin filtros)');
    
    // Test 3: Properties con un filtro simple
    await testEndpoint(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.PROPERTIES}?name=test`, 'Properties (con filtro name)');
    ...
    await testEndpoint(`${API_CONFIG.BASE_URL}/swagger`, 'Swagger Documentation');
    ...
    <Card title="Diagnóstico de API" className="mb-6">
      <div className="space-y-4">
        <div className="flex space-x-2">
          <Button onClick={runDiagnostic} disabled={isRunning} isLoading={isRunning}>
        ...
        <div className="text-sm text-gray-600">
          <p><strong>URL Base del Backend:</strong> {API_CONFIG.BASE_URL}</p>
          <p><strong>Endpoint de Propiedades:</strong> {API_CONFIG.ENDPOINTS.PROPERTIES}</p>
        </div>
              ...
              <div key={index} className={`p-3 rounded border-l-4 ${
                ...
                <div className="text-sm">
                  {result.statusCode && (
                    <span className={`inline-block px-2 py-1 rounded text-xs mr-2 ${
                      result.statusCode < 300 ? 'bg-green-200 text-green-800' :
                      result.statusCode < 400 ? 'bg-yellow-200 text-yellow-800' :
                ...
                {result.data !== null && result.data !== undefined && (
                  <details className="mt-2">
                    <summary className="text-xs text-gray-600 cursor-pointer">Ver respuesta</summary>
                    <pre className="text-xs bg-gray-100 p-2 rounded mt-1 overflow-auto max-h-32">
                      {String(typeof result.data === 'string' 
                        ? result.data 
        ...
        <div className="text-xs text-gray-500 space-y-1">
          <p><strong>Posibles soluciones para error 400:</strong></p>
          <ul className="list-disc list-inside space-y-1 ml-2">
            <li>Verificar que el backend esté ejecutándose en el puerto 5179</li>
            <li>Revisar que los parámetros de filtro sean los esperados por la API</li>
